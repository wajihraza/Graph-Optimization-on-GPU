...
// CPU DFS for comparison (like in BFS)
void dfsCPU(const Graph& graph, int startNode, std::vector<int>& levels) {
...
void dfsCUDA(const Graph& graph, int startNode, std::vector<int>& levels) {
    ...
    int startVal = 0;
    int visitedVal = 1;
    CUDA_CHECK(cudaMemcpy(d_levels + startNode, &startVal, sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_visited + startNode, &visitedVal, sizeof(int), cudaMemcpyHostToDevice));
    ...
    while (nextLevelSize > 0) {
        ...
        dfsKernel<<<numBlocks, blockSize>>>(d_edges, d_offset, d_visited, d_levels, currentLevel, d_nextLevelSize, numNodes);
...
int main() {
    // You can change the graph size for any code testing while marking
    int numNodes = 200000;   
    int edgesPerNode = 16;  
    int startNode = 0;
    ...
    for (int i = 1; i <= numNodes; i++) {
        offset[i] = offset[i - 1] + edgesPerNode;
    }
    ...
    dfsCPU(graph, startNode, levelsCPU);
    ...
    dfsCUDA(graph, startNode, levelsGPU);
